...
//Thanks to sjaak327 for working struct definition
struct HeadBlend {
	int shapeFirstID;
	char Padding1[4];
	int shapeSecondID;
...
void getFreeModeFace(PedSkin & skin, int type, bool global){
	...
	if (global){
		...
		if (temp.shapeFirstID >= PED::_GET_FIRST_PARENT_ID_FOR_PED_TYPE(3))
			skin.freemodeData.mother = temp.shapeFirstID - PED::_GET_FIRST_PARENT_ID_FOR_PED_TYPE(3) + PED::_GET_NUM_PARENT_PEDS_OF_TYPE(1);
		else
			skin.freemodeData.mother = temp.shapeFirstID - PED::_GET_FIRST_PARENT_ID_FOR_PED_TYPE(1);
		if (temp.shapeSecondID >= PED::_GET_FIRST_PARENT_ID_FOR_PED_TYPE(2))
			skin.freemodeData.father = temp.shapeFirstID - PED::_GET_FIRST_PARENT_ID_FOR_PED_TYPE(2) + PED::_GET_NUM_PARENT_PEDS_OF_TYPE(0);
		else
			skin.freemodeData.father = temp.shapeSecondID - PED::_GET_FIRST_PARENT_ID_FOR_PED_TYPE(0);
		skin.freemodeData.faceRatio = SYSTEM::ROUND(temp.shapeMix * 16);
		skin.freemodeData.skinRatio = SYSTEM::ROUND(temp.skinMix * 16);
